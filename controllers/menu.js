...
const ordercreate = async (req, res) => {
  ...
  try {
    const order_data = await orders.create({
      ...user_order,
      orderId: uuidv4(),
    });
...
const orderlist = async (req, res) => {
  let { limit, offset, userId } = req.params;
  offset = (offset - 1) * limit;
  try {
    const data = await orders.find({ userId }).skip(offset).limit(limit);
...
const productId = async (req, res) => {
  ...
  try {
    const data = await products.findOne({ productId });
